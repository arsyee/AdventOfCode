...
public class Solution12 {
	...
	public static IntPair calculate(List<String> input) {
		...
		reduceEquivalenceClasses(sections);
		return new IntPair(sections.get(new Integer(0)).size(), sections.size());
	}

	public static void reduceEquivalenceClasses(HashMap<Integer, HashSet<Integer>> sections) {
		int size = 0;
		eliminate: do {
			print(sections);
			size = sections.size();
			for (Entry<Integer, HashSet<Integer>> section : sections.entrySet()) {
				for (Integer value : section.getValue()) {
					if (!section.getKey().equals(value) && sections.keySet().contains(value)) {
						if (debug) System.out.println("moving "+value+" to "+section.getKey());
	...
	static void print(HashMap<Integer, HashSet<Integer>> sections) {
		if (!debug) return;
